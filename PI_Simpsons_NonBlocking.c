...
int main(int argc, char* argv[])
{
    ...
    while(!exit)
    {
      if (rank == 0)
      {
        ...
        for(i = 1; i< procs;i++)
        {
           rc = MPI_Isend(&n2, 1, MPI_INT, i, tag, MPI_COMM_WORLD, &request1); 
        ...
        for(j = 1; j< procs; j++)
        {
          rc = MPI_Irecv(&tmp, 1, MPI_DOUBLE, j, tag, MPI_COMM_WORLD,&request2);
     ...
     else
     {
      ...
      rc = MPI_Irecv(&n, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &request2);
      MPI_Wait(&request2,  &stat);
      if(n == -1)
        break;
      h = (ul - ll) / (double)n;
      ...
      rc = MPI_Isend(&partialRes, 1, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD, &request1);
      MPI_Wait(&request1,  &stat);
